...
library LibACL {
    ...
    function _assignRole(bytes32 _objectId, bytes32 _contextId, bytes32 _roleId) internal {
        ...
        if (
            _contextId == LC.SYSTEM_IDENTIFIER_BYTES32 && oldRole == LibHelpers._stringToBytes32(LC.ROLE_SYSTEM_ADMIN)
                && oldRole != _roleId
        ) {
        ...
        if (_contextId == LC.SYSTEM_IDENTIFIER_BYTES32 && _roleId == LibHelpers._stringToBytes32(LC.ROLE_SYSTEM_ADMIN))
        {
            if (
                LibHelpers._isAddress(_objectId)
                    && LibDiamond.contractOwner() == LibHelpers._getAddressFromId(_objectId)
            ) {
    ...
    function _unassignRole(bytes32 _objectId, bytes32 _contextId) internal {
        ...
        if (_contextId == LC.SYSTEM_IDENTIFIER_BYTES32 && roleId == LibHelpers._stringToBytes32(LC.ROLE_SYSTEM_ADMIN)) {
            require(s.sysAdmins > 1, "must have at least one system admin");
            unchecked {
                s.sysAdmins -= 1;
            }
        ...
        returns (bool)
    {
        ...
        if (_isInGroup(_assignerId, LC.SYSTEM_IDENTIFIER_BYTES32, assignerGroup)) return true;
        return false;
    }
