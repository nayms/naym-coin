    ...
    for (const { verifierName, verifierUrl, verifierApiKey } of verifiers) {
        ...
        for (const { name, onChain } of contracts) {
            ...
            if (onChain.constructorArgs.length) {
                args = (await execa.command(`cast abi-encode constructor(address) ${onChain.constructorArgs.join(" ")}`)).stdout;
            }

            console.log(`Verifying ${name} at ${onChain.address} with args ${args}`);

            await execa.command(
                `forge v ${onChain.address} ${name} --constructor-args ${args} --chain-id ${deploymentInfo[target].chainId} ${verificationArg} --etherscan-api-key ${apiKey} --watch`,
            );
